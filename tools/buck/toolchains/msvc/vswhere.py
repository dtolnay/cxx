...
def find_with_vswhere_exe():
    ...
    if program_files is None:
        print("expected a %ProgramFiles(x86)% or %ProgramFiles% environment variable", file=sys.stderr)
        sys.exit(1)

    vswhere_exe = Path(program_files) / "Microsoft Visual Studio" / "Installer" / "vswhere.exe"
    ...
    # Version is a sequence of 16-bit integers (at most 4 of them?).
    ...
    for vs_instance in list(vswhere_json):
        ...
        version_file = installation_path / "VC" / "Auxiliary" / "Build" / "Microsoft.VCToolsVersion.default.txt"
    ...
    print("vswhere.exe did not find a suitable MSVC toolchain containing cl.exe, lib.exe, ml64.exe", file=sys.stderr)
    sys.exit(1)
...
def get_sdk10_dir():
    ...
    registry_key = winreg.OpenKey(registry, key_name, access=winreg.KEY_READ | winreg.KEY_WOW64_32KEY)
    installation_folder = Path(winreg.QueryValueEx(registry_key, "InstallationFolder")[0])
...
def main():
    ...
    to_json = lambda tool: json.dumps(
        tool._asdict(),
        indent=4,
        default=lambda path: str(path),
    )

    output.cl.write(to_json(cl_exe))
    output.lib.write(to_json(lib_exe))
    output.ml64.write(to_json(ml64_exe))
