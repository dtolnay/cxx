...
pub unsafe trait ConstPtr: Sized {
    #[doc(hidden)]
    fn __typename(f: &mut fmt::Formatter) -> fmt::Result;
    #[doc(hidden)]
...
unsafe impl<T: ConstPtr> VectorElement for *const T {
    fn __typename(f: &mut fmt::Formatter) -> fmt::Result {
        T::__typename(f)
    }
...
unsafe impl<T: MutPtr> VectorElement for *mut T {
    fn __typename(f: &mut fmt::Formatter) -> fmt::Result {
        T::__typename(f)
    }
...
macro_rules! ptr_by_value_methods {
...
macro_rules! impl_ptrc_vector_element {
    ($kind:ident, $segment:expr, $name:expr, $ty:ty) => {
        ...
        unsafe impl ConstPtr for $ty {
            ...
            ptr_by_value_methods!($kind, $segment, "ptrc", *const $ty);
            fn __unique_ptr_null() -> MaybeUninit<*mut c_void> {
                extern "C" {
...
macro_rules! impl_ptrm_vector_element {
    ($kind:ident, $segment:expr, $name:expr, $ty:ty) => {
        ...
        unsafe impl MutPtr for $ty {
            ...
            ptr_by_value_methods!($kind, $segment, "ptrm", *mut $ty);
            fn __unique_ptr_null() -> MaybeUninit<*mut c_void> {
                extern "C" {
...
macro_rules! impl_ptr_vector_element_for_primitive {
    (ptrc, $ty:ident) => {
        impl_ptrc_vector_element!(trivial, stringify!($ty), stringify!($ty), $ty);
    };
    (ptrm, $ty:ident) => {
        impl_ptrm_vector_element!(trivial, stringify!($ty), stringify!($ty), $ty);
    };
}
