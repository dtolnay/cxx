    ...
    match &ty.inner {
        Type::Ident(ident) => {
            ...
            match Atom::from(&ident.rust) {
                ...
                | Some(F64) | Some(RustString) | Some(CInt) | Some(CDouble) | Some(CFloat)
                | Some(CLong) | Some(CLongLong) | Some(CSChar) | Some(CShort) | Some(CUChar)
                | Some(CUInt) | Some(CULong) | Some(CULongLong) | Some(CUShort) => return,
                Some(Bool) => { /* todo */ }
                Some(CxxString) => {}
            }
        }
...
fn check_type_shared_ptr(cx: &mut Check, ptr: &Ty1) {
    if let Type::Ident(ident) = &ptr.inner {
        ...
        match Atom::from(&ident.rust) {
            ...
            | Some(I8) | Some(I16) | Some(I32) | Some(I64) | Some(Isize) | Some(F32)
            | Some(F64) | Some(CxxString) | Some(CInt) | Some(CDouble) | Some(CFloat)
            | Some(CLong) | Some(CLongLong) | Some(CSChar) | Some(CShort) | Some(CUChar)
            | Some(CUInt) | Some(CULong) | Some(CULongLong) | Some(CUShort) => return,
...
fn check_type_cxx_vector(cx: &mut Check, ptr: &Ty1) {
    if let Type::Ident(ident) = &ptr.inner {
        ...
        match Atom::from(&ident.rust) {
            ...
            | Some(CxxString) | Some(CInt) | Some(CDouble) | Some(CFloat) | Some(CLong)
            | Some(CLongLong) | Some(CSChar) | Some(CShort) | Some(CUChar) | Some(CUInt)
            | Some(CULong) | Some(CULongLong) | Some(CUShort) => return,
            Some(Char) => { /* todo */ }
