...
#[derive(Default)]
pub struct TrivialReasons<'a> {
    reasons: Vec<TrivialReason<'a>>,
}
...
pub(crate) fn required_trivial_reasons<'a>(
...
) -> UnorderedMap<&'a Ident, TrivialReasons<'a>> {
    ...
    let mut insist_extern_types_are_trivial = |ident: &'a NamedType, reason| {
            ...
            && !enums.contains_key(&ident.rust)
        {
            required_trivial
                .entry(&ident.rust)
                .or_insert_with(TrivialReasons::default)
                .reasons
                .push(reason);
        }
    };
...
impl<'a> TrivialReasons<'a> {
    #[allow(dead_code)] // only used by cxx-build, not cxxbridge-macro
    pub(crate) fn iter(&'a self) -> impl Iterator<Item = &'a TrivialReason<'a>> + 'a {
        self.reasons.iter()
    }

    pub(crate) fn as_what(&'a self, name: &'a Pair) -> impl Display + 'a {
