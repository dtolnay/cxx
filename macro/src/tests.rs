...
fn test_struct_with_lifetime() {
    ...
    // This is mostly a regression test for problems that were accidentally introduced
    // in https://github.com/dtolnay/cxx/pull/1658 which for the input above would
    // generate syntatically invalid code:
    //
    //     impl<'a> ::cxx::memory::UniquePtrTarget for StructWithLifetime < > < 'a > {
    //
    // In presence of syntax error the test helper `bridge` called above will panic,
    // but for completeness the assertion below verifies that correct code has been generated.
    assert!(rs.contains("impl<'a> ::cxx::memory::UniquePtrTarget for StructWithLifetime<'a> {"));

    // Assertions for to other places that refer to `StructWithLifetime`.
